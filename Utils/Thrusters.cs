...
namespace SE_Script_Library.Utils
{
    public class Thrusters : ReferenceOrientedBlocks
    {
        ...
        public Thrusters(ref IMyTerminalBlock referenceBlock, List<IMyTerminalBlock> blocks)
            : base(ref referenceBlock)
        {
            Update(blocks);

            IMyThrust thrust = thrusters.Values.First().First();
        ...
        private void Accelerate(VRageMath.Vector3 dir, float value)
        {
            if (value < MinAcceleration || value > MaxAcceleration)
                throw new Exception("Value out of range [" + MinAcceleration + ", " + MaxAcceleration + "].");
